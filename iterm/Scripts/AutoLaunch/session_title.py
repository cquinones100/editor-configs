...
def get_color_key(cwd):
    if not cwd:
        return ""
    return get_title(cwd)


def get_tab_color(cwd):
    key = get_color_key(cwd)
    if not key:
        return iterm2.Color(128, 128, 128)

    hue = int.from_bytes(hashlib.sha256(key.encode()).digest()[:4], "big") % 360
    h_segment = hue // 60
...
async def update_session_color(app, session_id, path):
    if not path:
        return
...
async def monitor_session_path(connection, session_id):
    app = await iterm2.async_get_app(connection)

    async with iterm2.VariableMonitor(
        connection, iterm2.VariableScopes.SESSION, "path", session_id
    ) as mon:
        # Set color from current path now that the monitor is listening,
        # so any change that arrives after this point is guaranteed to be caught.
        session = app.get_session_by_id(session_id)
        if session:
            path = await session.async_get_variable("path")
            await update_session_color(app, session_id, path)

        while True:
            new_value = await mon.async_get()
            await update_session_color(app, session_id, new_value)
...
async def main(connection):
    ...
    await iterm2.EachSessionOnceMonitor.async_foreach_session_create_task(
        app, lambda session_id: monitor_session_path(connection, session_id)
    )
